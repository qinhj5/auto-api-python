...
def create_package_dir(name: str) -> None:
    """
    ...
    api_dir = os.path.abspath(os.path.join(current_dir, f"../tmp/api/{name}"))
    os.makedirs(api_dir, exist_ok=True)
    init_path = os.path.abspath(os.path.join(api_dir, f"__init__.py"))
    with open(init_path, "w", encoding="utf-8") as f:
        f.write("# -*- coding: utf-8 -*-\n")

    testcases_dir = os.path.abspath(os.path.join(current_dir, f"../tmp/testcases/{name}"))
    os.makedirs(testcases_dir, exist_ok=True)
    init_path = os.path.abspath(os.path.join(testcases_dir, f"__init__.py"))
    with open(init_path, "w", encoding="utf-8") as f:
        f.write("# -*- coding: utf-8 -*-\n")

    init_path = os.path.abspath(os.path.join(current_dir, f"../tmp/__init__.py"))
    with open(init_path, "w", encoding="utf-8") as f:
        f.write("# -*- coding: utf-8 -*-\n")
...
def pascal_to_snake(name: str) -> str:
    ...
    """
    name = remove_non_alphabet_from_start(name.strip())
...
def get_swagger_json(swagger_url: str) -> Generator[tuple, None, None]:
    """
    Get Swagger JSON data by making a request to the specified Swagger URL.

    Args:
        swagger_url (str): The URL of the Swagger documentation.
    ...
    """
    response = requests.get(swagger_url, headers=Global.constants.HEADERS)
...
def get_swagger_dict(swagger_url: str) -> dict:
    """
    ...
    """
    json_generator = get_swagger_json(swagger_url)
    swagger_dict = {}
    for uri, api_details in json_generator:
        for api_method, api_detail in api_details.items():
            module_name = pascal_to_snake(api_detail["tags"][0])
            create_package_dir(module_name)
...
def convert_path_params(path: str) -> str:
    ...
    for match in matches:
        path = path.replace(match, avoid_keywords(pascal_to_snake(match)))
    return path
...
def deduplicated_params(params: list) -> list:
    ...
    for param in params:
        ...
        snake_name = pascal_to_snake(param["name"])
...
def get_api_func(api: dict) -> Tuple[str, bool]:
    ...
    if snake_name.startswith(method):
        ...
        func_name = f"""{method}_{pascal_to_snake(api["detail"]["operationId"])}"""

    print(json.dumps(api["detail"]))

    summary = api["detail"].get("summary", "Null")

    summary = get_wrapped_string(summary)

    uri = convert_path_params(api["uri"])

    params = api["detail"].get("parameters", [])
    if params:
        params = deduplicated_params(params)
    ...
    use_list = False
    for param in params:
        param_name = pascal_to_snake(param["name"])
        param_name = avoid_keywords(param_name)
        param_type = get_python_type(param.get("type"))

        if param.get("type") == "array":
            list_inner_type = get_python_type(param["items"]["type"])
    ...
    func_tail = f"""        return self.send_prepared_request(uri=f"{uri}", method="{method.upper()}"{request_str})\n"""
...
def write_api_file(module: str, module_code: str) -> None:
    """
    ...
    """
    current_dir = os.path.dirname(__file__)
    api_dir = os.path.abspath(os.path.join(current_dir, f"../tmp/api/{module}"))
...
def generate_api_templates(swagger_dict: dict) -> None:
    """
    ...
    for module in swagger_dict.keys():
        ...
        for api in swagger_dict[module]:
            func_code, use_list = get_api_func(api)
            if use_list:
                import_list = True
            module_code += func_code
        module_code = get_api_header(snake_to_pascal(module) + "API", import_list) + module_code
        write_api_file(module, module_code)
...
def get_conf_code(module: str) -> str:
    ...
    """
    api_cls = f"{snake_to_pascal(module)}API"
    conf_code = ""
    ...
    conf_code += f"    {module}_api = {api_cls}(base_url=Global.constants.BASE_URL, headers=Global.constants.HEADERS)\n"
    conf_code += f"    return {module}_api\n"
    return conf_code


def write_conf_file(module: str, conf_code: str) -> None:
    """
    ...
    """
    current_dir = os.path.dirname(__file__)
    testcases_dir = os.path.abspath(os.path.join(current_dir, f"../tmp/testcases/{module}"))
...
def get_testcases_code(module: str, api: dict) -> Tuple[str, str]:
    ...
    if snake_name.startswith(method):
        ...
        api_func_name = f"""{method}_{pascal_to_snake(api["detail"]["operationId"])}"""
        test_func_name = f"""test_{api_func_name}"""
...
def write_testcases_file(module: str, file_name: str, testcases_code: str) -> None:
    """
...
def generate_testcases_templates(swagger_dict: dict) -> None:
    """
    ...
    """
    for module in swagger_dict.keys():
        conf_code = get_conf_code(module)
        write_conf_file(module, conf_code)
        for api in swagger_dict[module]:
            testcases_code, file_name = get_testcases_code(module, api)
            write_testcases_file(module, file_name, testcases_code)


def generate_templates(swagger_url: str) -> None:
    """
    ...
    """
    clear_tmp_dir()
    swagger_dict = get_swagger_dict(swagger_url)
    generate_api_templates(swagger_dict)
    generate_testcases_templates(swagger_dict)


if __name__ == "__main__":
    # swagger_url is the link to the swagger api-docs
    generate_templates(swagger_url="")
